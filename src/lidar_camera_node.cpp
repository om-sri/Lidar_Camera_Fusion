...
float maxlen =100.0;       //maximum distance from lidar
float minlen = 0.01;     //minimum distance from lidar
float max_FOV = 3.0;    // maximum angle in radians of camera
float min_FOV = 0.4;    // maniimum angle in radians of camera

/// parameters for converting point to image
...
//in_pc2 is pointer to lidardata, in_image is pointer for camera data
{
 ...
 // pcl::PointXYZI point_aux;
  ...
  // creating a range image from the filtered point cloud data (cloud_out)
  ...
  arma::mat Z;  // interpolation of image
  arma::mat Zz; 
